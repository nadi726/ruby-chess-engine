...
RSpec.describe Game::Query do
  ...
  describe 'with invalid arguments' do
    ...
    it 'for #piece_can_move?' do
      invalid_squares.each do |invalid_square|
        expect(start_query.piece_can_move?(invalid_square, valid_square)).to eq(klass::INVALID_ARGUMENT)
        expect(start_query.piece_can_move?(valid_square, invalid_square)).to eq(klass::INVALID_ARGUMENT)
      end
    end

    it 'for #piece_attacking?' do
      invalid_squares.each do |invalid_square|
        expect(start_query.piece_attacking?(invalid_square, valid_square)).to eq(klass::INVALID_ARGUMENT)
        expect(start_query.piece_attacking?(valid_square, invalid_square)).to eq(klass::INVALID_ARGUMENT)
      end
    end

    it 'for #square_attacked?' do
      invalid_squares.each do |invalid_square|
        expect(start_query.square_attacked?(invalid_square, :white)).to eq(klass::INVALID_ARGUMENT)
      end

      invalid_colors.each do |invalid_color|
        expect(start_query.square_attacked?(invalid_color)).to eq(klass::INVALID_ARGUMENT)
      end
    end

    it 'for #legal moves' do
      invalid_colors.each do |invalid_color|
        expect(start_query.legal_moves(invalid_color)).to eq(klass::INVALID_ARGUMENT)
      end
    end

    it 'for #in_check?' do
      invalid_colors.each do |invalid_color|
        expect(start_query.in_check?(invalid_color)).to eq(klass::INVALID_ARGUMENT)
      end
    end
  end
  ...
  describe '#square_attacked?' do
    ...
    it 'returns true for an occupied square attacked by a piece of the other color' do
      ...
      query = klass.new(Position.start.with(board: new_board))
  ...
  describe '#legal moves' do
    ...
    it 'returns special moves when available' do
      ...
      history = Game::History[moves: [MovePieceEvent[Piece[:black, :pawn], Square[:b, 7], Square[:b, 5]]]]
      query = klass.new(position, history)
  ...
  describe '#in_check?' do
    context 'with no check' do
      ...
      it 'returns false after move events sequence' do
        ...
        state = event_history.reduce(Game::State.new(position:
      ...
      it 'returns false if en passant is available but does not expose king to check' do
        ...
        state = Game::State.new(position: position)
  ...
  describe '#in_checkmate?' do
    ...
    it "Detects checkmate based on move sequence (fool's mate)" do
      ...
      state = event_history.reduce(Game::State.new(
  ...
  describe '#insufficient_material?' do
    def query_for(pieces)
      ...
      klass.new(Position.start.with(castling_rights: CastlingRights.none, board: board))
    end
  ...
  describe '#threefold_repetition?' do
    ...
    it 'returns true for the current square being repeated 3 times' do
      ...
      query = klass.new(position, Game::History.start.with(position_signatures: position_signatures))
    ...
    it 'returns false for the previous position being repeated 3 or more times' do
      ...
      query = klass.new(current_position, Game::History.start.with(position_signatures: position_signatures))
      expect(query).not_to be_threefold_repetition
    end
  end
end
