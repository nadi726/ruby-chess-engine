# frozen_string_literal: true

require 'game_state/game_query'
require 'game_state/board'
require 'game_state/position'
require 'event_handlers/move_event_handler'
require 'data_definitions/events'
require 'data_definitions/square'

RSpec.describe MoveEventHandler do
  # A board & query setup used for most of the tests
  let(:board) do
    fill_board(
      # White pieces
      [
        [Piece[:white, :king], Square[:e, 1]],
        [Piece[:white, :rook],   Square[:a, 1]],
        [Piece[:white, :pawn],   Square[:d, 5]],
        [Piece[:white, :knight], Square[:b, 1]],
        # Black pieces
        [Piece[:black, :king],   Square[:e, 8]],
        [Piece[:black, :queen],  Square[:d, 8]],
        [Piece[:black, :pawn],   Square[:c, 6]],
        [Piece[:black, :rook],   Square[:h, 8]]
      ]
    )
  end

  let(:white_query) do
    GameQuery.new(Position.start.with(board: board))
  end

  let(:black_query) do
    GameQuery.new(white_query.position.with(current_color: :black))
  end

  context 'move event' do
    it 'returns valid result for 1-rank pawn move' do
      main = MovePieceEvent[Square[:d, 2], Square[:d, 3], nil]
      handler = MoveEventHandler.new(start_query, main, [])
      expect(handler.process).to be_a_successful_handler_result
    end

    it 'returns valid result for 2-rank pawn move' do
      main = MovePieceEvent[Square[:f, 2], Square[:f, 4], nil]
      handler = MoveEventHandler.new(start_query, main, [])
      expect(handler.process).to be_a_successful_handler_result
    end

    it 'returns invalid result for 3-rank pawn move' do
      main = MovePieceEvent[Square[:g, 2], Square[:f, 5], nil]
      handler = MoveEventHandler.new(start_query, main, [])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'returns valid result for valid knight move' do
      main = MovePieceEvent[Square[:b, 1], Square[:c, 3], nil]
      handler = MoveEventHandler.new(start_query, main, [])
      expect(handler.process).to be_a_successful_handler_result
    end
  end

  context 'for complex state' do
    it 'allows white rook to move to a5' do
      main = MovePieceEvent[Square[:a, 1], Square[:a, 5], nil]
      handler = MoveEventHandler.new(white_query, main, [])
      expect(handler.process).to be_a_successful_handler_result
    end

    it 'prevents white pawn from moving to c6 (blocked by black pawn)' do
      event = MovePieceEvent[Square[:d, 5], Square[:c, 6], nil]
      handler = MoveEventHandler.new(white_query, event, [])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'allows white knight to jump to c3' do
      event = MovePieceEvent[Square[:b, 1], Square[:c, 3], nil]
      handler = MoveEventHandler.new(white_query, event, [])
      expect(handler.process).to be_a_successful_handler_result
    end

    it 'prevents white rook from moving through black queen' do
      event = MovePieceEvent[Square[:a, 1], Square[:d, 8], nil]
      handler = MoveEventHandler.new(white_query, event, [])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'allows black queen to capture white pawn on d5 with empty remove event' do
      event = MovePieceEvent[Square[:d, 8], Square[:d, 5], nil]
      handler = MoveEventHandler.new(black_query, event, [RemovePieceEvent.new])
      expect(handler.process).to be_a_successful_handler_result
    end

    it 'fails when capturing but no RemovePieceEvent is given' do
      event = MovePieceEvent[Square[:d, 8], Square[:d, 5], nil]
      handler = MoveEventHandler.new(black_query, event, [])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'prevents black rook from moving to a1 (blocked by white rook)' do
      event = MovePieceEvent[Square[:h, 8], Square[:a, 1], nil]
      handler = MoveEventHandler.new(black_query, event, [])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'fails when RemovePieceEvent square does not match the target square' do
      move = MovePieceEvent[Square[:d, 8], Square[:d, 5], nil]
      remove = RemovePieceEvent[Square[:c, 6], Piece[:white, :pawn]]
      handler = MoveEventHandler.new(black_query, move, [remove])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'fails when RemovePieceEvent piece is not actually at given square' do
      move = MovePieceEvent[Square[:d, 8], Square[:d, 5], nil]
      remove = RemovePieceEvent[Square[:d, 5], Piece[:white, :knight]]
      handler = MoveEventHandler.new(black_query, move, [remove])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'fails when moving from an empty square' do
      event = MovePieceEvent[Square[:c, 4], Square[:d, 5], nil]
      handler = MoveEventHandler.new(white_query, event, [])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'fails when trying to move onto allied piece even with RemovePieceEvent' do
      move = MovePieceEvent[Square[:a, 1], Square[:d, 5], nil]
      remove = RemovePieceEvent[Square[:d, 5], Piece[:white, :pawn]]
      handler = MoveEventHandler.new(white_query, move, [remove])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'fails if RemovePieceEvent is given but no piece is being captured' do
      move = MovePieceEvent[Square[:a, 1], Square[:a, 4], nil]
      remove = RemovePieceEvent[Square[:a, 4], nil]
      handler = MoveEventHandler.new(white_query, move, [remove])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'succeeds when a valid capture is declared correctly & fully' do
      move = MovePieceEvent[Square[:d, 8], Square[:d, 5], nil]
      captured = board.get(Square[:d, 5])
      remove = RemovePieceEvent[Square[:d, 5], captured]
      handler = MoveEventHandler.new(black_query, move, [remove])
      expect(handler.process).to be_a_successful_handler_result
    end
  end

  context 'for another complex state' do
    let(:board) do
      fill_board(
        [
          [Piece[:white, :king], Square[:e, 1]],
          [Piece[:white, :rook],   Square[:a, 1]],
          [Piece[:white, :pawn],   Square[:d, 5]],
          [Piece[:white, :knight], Square[:b, 1]],
          [Piece[:white, :bishop], Square[:c, 1]],
          [Piece[:black, :king],   Square[:e, 8]],
          [Piece[:black, :queen],  Square[:d, 8]],
          [Piece[:black, :pawn],   Square[:a, 6]],
          [Piece[:black, :rook],   Square[:h, 8]],
          [Piece[:black, :bishop], Square[:f, 8]]
        ]
      )
    end
    let(:white_query) do
      GameQuery.new(Position.start.with(board: board))
    end

    let(:black_state) do
      GameQuery.new(white_query.position.with(current_color: :black))
    end

    context 'friendly fire prevention' do
      it 'prevents moving onto a square occupied by own piece' do
        # White knight tries to move to c1 (occupied by white bishop)
        event = MovePieceEvent[Square[:b, 1], Square[:c, 1], nil]
        handler = MoveEventHandler.new(white_query, event, [])
        expect(handler.process).to be_a_failed_handler_result
      end

      it 'prevents capturing own piece even with remove event' do
        # White rook tries to capture white pawn on d5
        move_event = MovePieceEvent[Square[:a, 1], Square[:d, 5], nil]
        remove_event = RemovePieceEvent[Square[:d, 5], Piece[:white, :pawn]]
        handler = MoveEventHandler.new(white_query, move_event, [remove_event])
        expect(handler.process).to be_a_failed_handler_result
      end
    end

    context 'capture enforcement' do
      it 'requires remove event to capture opponent piece' do
        # White rook tries to capture black pawn on a6 without remove event
        event = MovePieceEvent[Square[:a, 1], Square[:a, 6], nil]
        handler = MoveEventHandler.new(white_query, event, [])
        expect(handler.process).to be_a_failed_handler_result
      end

      it 'allows capture with correct remove event' do
        move_event = MovePieceEvent[Square[:a, 1], Square[:a, 6], nil]
        remove_event = RemovePieceEvent[Square[:a, 6], nil]
        handler = MoveEventHandler.new(white_query, move_event, [remove_event])
        expect(handler.process).to be_a_successful_handler_result
      end
    end

    context 'bishop movement and path blocking' do
      it 'allows bishop to move diagonally unobstructed' do
        # Move white bishop from c1 to e3 (assuming clear path)
        event = MovePieceEvent[Square[:c, 1], Square[:e, 3], nil]
        handler = MoveEventHandler.new(white_query, event, [])
        expect(handler.process).to be_a_successful_handler_result
      end

      it 'prevents bishop moving through piece blocking path' do
        # Place a piece blocking bishop at d2
        board_with_block = board.insert(Piece.new(:white, :pawn), Square[:d, 2])
        query_with_block = GameQuery.new(Position.start.with(board: board_with_block))
        event = MovePieceEvent[Square[:c, 1], Square[:e, 3]]
        handler = MoveEventHandler.new(query_with_block, event, [])
        expect(handler.process).to be_a_failed_handler_result
      end
    end

    context 'turn enforcement' do
      it 'prevents black from moving when it is white’s turn' do
        event = MovePieceEvent[Square[:e, 8], Square[:e, 7]]
        handler = MoveEventHandler.new(white_query, event, [])
        expect(handler.process).to be_a_failed_handler_result
      end

      it 'allows black to move on black’s turn' do
        event = MovePieceEvent[Square[:e, 8], Square[:e, 7]]
        handler = MoveEventHandler.new(black_state, event, [])
        expect(handler.process).to be_a_successful_handler_result
      end
    end
  end

  context 'en passant' do
    let(:black_pawn_move) do
      MovePieceEvent[Square[:d, 7], Square[:d, 5], Piece[:black, :pawn]]
    end

    let(:new_board) do
      fill_board(
        [
          [Piece[:white, :pawn], Square[:e, 5]],
          [Piece[:black, :pawn], Square[:d, 5]]
        ],
        board: board.remove(Square[:d, 5])
      )
    end

    let(:new_move_history) do
      white_query.move_history.add(Immutable::List[black_pawn_move])
    end

    let(:query) do
      GameQuery.new(
        Position.start.with(board: new_board, en_passant_target: Square[:d, 6]),
        new_move_history
      )
    end
    it 'rejects en passant without required events' do
      # Attempt en passant without RemovePieceEvent
      move_event = MovePieceEvent[Square[:e, 5], Square[:d, 6]]
      handler = MoveEventHandler.new(query, move_event, [])
      expect(handler.process).to be_a_failed_handler_result
    end

    it 'accept en passant with required events from move event' do
      move_event = MovePieceEvent[Square[:e, 5], Square[:d, 6]]
      handler = MoveEventHandler.new(query, move_event, [RemovePieceEvent.new])
      expect(handler.process).to be_a_successful_handler_result
    end
  end

  context 'check detection' do
    it 'continues silently when the king is currently in check but moves out of it' do
      board = fill_board(
        [
          [Piece[:black, :queen], Square[:e, 8]],
          [Piece[:black, :king], Square[:d, 8]],
          [Piece[:white, :king], Square[:e, 1]]
        ]
      )

      position = Position.start.with(board: board)
      state = GameState.new(position: position)
      move_event = MovePieceEvent[Square[:e, 1], Square[:f, 1], nil]
      handler = MoveEventHandler.new(state.query, move_event, [])

      expect(handler.process).to be_a_successful_handler_result
    end

    it 'fails when moving into check' do
      board = fill_board(
        [
          [Piece[:black, :queen], Square[:e, 8]],
          [Piece[:black, :king], Square[:d, 8]],
          [Piece[:white, :rook], Square[:e, 4]],
          [Piece[:white, :king], Square[:e, 1]]
        ]
      )

      position = Position.start.with(board: board)
      state = GameState.new(position: position)
      move_event = MovePieceEvent[Square[:e, 4], Square[:b, 4], nil]
      handler = MoveEventHandler.new(state.query, move_event, [])

      expect(handler.process).to be_a_failed_handler_result
    end

    it "fails when the king is currently in check and doesn't moves out of it" do
      board = fill_board(
        [
          [Piece[:black, :queen], Square[:e, 8]],
          [Piece[:black, :king], Square[:d, 8]],
          [Piece[:white, :king], Square[:e, 1]]
        ]
      )

      position = Position.start.with(board: board)
      state = GameState.new(position: position)
      move_event = MovePieceEvent[Square[:e, 1], Square[:e, 2], nil]
      handler = MoveEventHandler.new(state.query, move_event, [])

      expect(handler.process).to be_a_failed_handler_result
    end
  end
end
