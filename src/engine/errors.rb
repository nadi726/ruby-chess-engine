...
# which are a regular part of the control flow, and returned to the listeners in the form of `TurnResult.error`.
...
# Raised when attempting to use an invalid `Position`.
# Note that positions can be created invalidly by design, but using them is forbidden.
class InvalidPositionError < InvariantViolationError; end
